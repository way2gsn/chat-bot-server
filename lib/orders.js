...
// ── Public API ───────────────────────────────────────────────────────────────
async function saveOrder({ orderId, customerPhone, customerName, items, total, paymentMethod, address, lang, customerType }) {
  // If customerName is not passed, let's try to look it up from the users table
  ...
  if (!finalName) {
    try {
      const { data: userData } = await supabase
        ...
        .single();
      if (userData) finalName = userData.name;
    } catch (e) {
      // ignore
    }
  }
...
module.exports = { saveOrder, confirmOrder, cancelOrder, updateOrder, getAllOrders, getStats, deleteOrder };