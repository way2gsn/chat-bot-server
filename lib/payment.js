...
function generatePhonePeLink({ amount, orderId, note }) {
  ...
  const baseUrl = process.env.SERVER_URL || (process.env.RAILWAY_STATIC_URL ? `https://${process.env.RAILWAY_STATIC_URL}` : "http://localhost:3000");
  return `${baseUrl.replace(/\/$/, "")}/pay?url=${encoded}`;
}
