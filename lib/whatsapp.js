...
// Send products in a category — splits into sections of 10 (WhatsApp limit)
async function sendProductsMenu(to, products, lang, categoryName, ui, customerType = "retail") {
  ...
  // WhatsApp max 10 rows per section, max 10 sections
  const sections = [];
  for (let i = 0; i < rows.length; i += 10) {
    sections.push({
      title: rows.length > 10 ? `${categoryName} (${Math.floor(i/10)+1})` : categoryName,
      rows:  rows.slice(i, i + 10),
    });
  }

  // If too many sections send multiple messages
  ...
  // More than 100 products — send first 100 only
  return sendListMenu(to, {
    bodyText:    `📦 *${categoryName}*\nSelect a product:`,
    buttonLabel: ui("menu_btn"),
