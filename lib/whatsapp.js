...
async function apiCall(body) {
  const token = process.env.WHATSAPP_TOKEN || "EAAWYwXNG9j8BRvYB1oz9JhjjUrBbZC0aQZA9ntpzDyntmmahw3Aar0dWXeZCMXpDaZCI4fowyurWIe25OTHt28R4qtIotrZAAVIrkMry4R6iQNuxQXf4z7HJrpb11DhPYZAuxZCZAYerGi5zCXc8gsWxkqAnYbHwFj8hdhxV4N7YozCgTwmZB16CdT8z924815lLruwZDZD";
  const phoneId = process.env.WHATSAPP_PHONE_ID;
  const path = `/v19.0/${phoneId}/messages`;

  // Normalize recipient phone number: remove non-digits, and prepend 91 if it is 10 digits
  if (body.to) {
    let cleaned = String(body.to).replace(/\D/g, "");
...
async function sendMainMenu(to, lang, ui) {
  // Send welcome text first (company description) then menu — exactly like old bot
  await sendText(to, ui("welcome"));
  return sendListMenu(to, {
    ...
    sections: [{
      title: "Menu",
      rows: [
        { id: "action_shop", title: "Shop products", description: ui("shop_desc") },
        { id: "action_categories", title: "Explore categories", description: ui("categories_desc") },
        { id: "action_orders", title: "Order history", description: ui("orders_desc") },
        { id: "action_support", title: "Customer care", description: ui("support_desc") },
      ],
    }],
  });
}
...
// Send WhatsApp Approved Template (works for all users anytime)
async function sendTemplate(to, templateName, { parameters = [], hasQuickReply = false, language = "en", headerImage = null } = {}) {
  console.log("Sending Template:", templateName, "to:", to);
  const components = [];

  // Default image for phasal_bazar_shopping if no custom image is provided
  let finalHeaderImage = headerImage;
  if (templateName === "phasal_bazar_shopping" && !finalHeaderImage) {
    finalHeaderImage = "https://phasalbazar.com/cdn/shop/files/Workshops.png?v=1775062629&width=2000";
  }
  ...
  if (parameters.length > 0) {
    ...
    components.push({
      type: "body",
      parameters: parameters.map((val, idx) => {
        if (val && typeof val === "object") {
          const pName = val.parameter_name || val.name || paramNames[idx];
          const pText = val.text !== undefined ? val.text : val.value;
          const paramObj = { type: "text", text: String(pText) };
          if (pName) {
            paramObj.parameter_name = pName;
          }
          return paramObj;
        }
...
async function sendFlow(to, { flowId, headerText, bodyText, footerText, buttonText, screenName = "WELCOME", payload = {} }) {
  ...
  console.log("Flow result:", result.status, result.body?.substring(0, 100));
  return result;
}

// Send native WhatsApp catalog link
async function sendCatalogLink(to, lang) {
  const captions = {
    en: "Click on the view catalog button to explore products",
    hi: "Products dekhne ke liye neeche button click karein",
    ta: "பொருட்கள் பார்க்க கீழே உள்ள button click செய்யவும்",
    te: "ఉత్పత్తులు చూడటానికి కింద button click చేయండి",
  };
  return apiCall({
    to,
    type: "interactive",
    interactive: {
      type: "cta_url",
      body: { text: captions[lang] || captions.en },
      action: {
        name: "cta_url",
        parameters: {
          display_text: "View Catalogue",
          url: `https://wa.me/c/917771012123`,
        },
      },
    },
  });
...
async function sendUrlButton(to, { headerText, bodyText, footerText, buttonLabel, url }) {
  const interactive = {
    type: "cta_url",
    body: { text: bodyText },
    action: {
      name: "cta_url",
      parameters: {
        display_text: (buttonLabel || "Open Link").substring(0, 20),
        url: url
      }
    }
  };
...
module.exports = {
  ...
  sendFlow, sendTemplate, sendCatalogLink, sendUrlButton
};