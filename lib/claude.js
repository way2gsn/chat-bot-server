...
function buildSystemPrompt(lang, customerType = "retail") {
  return `You are a friendly WhatsApp shopping assistant for "Phasal Bazar" — an Indian agriculture and food products business selling fresh vegetables, grains, snacks (muruku, chakli, namkeen), pickles, and papad.

CATALOG:
${getCatalogText(lang, customerType)}
...
async function getAIReply(session, userMessage) {
  const { lang, messages, customerType = "retail" } = session;
  ...
  try {
    const data = await httpsPost({
      ...
      temperature: 0.7,
      messages: [
        { role: "system", content: buildSystemPrompt(lang, customerType) },
